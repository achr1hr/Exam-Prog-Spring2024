...
struct Edge {
  ...
  int64_t to;
  int64_t index;
  Edge(int64_t from, int64_t to, int64_t index)
...
class DirGraphAdjList {
 public:
  DirGraphAdjList(int64_t n_vertices, const std::vector<Edge>& edges)
 ...
 private:
  void DFS(int64_t v, int64_t parent, int64_t parent_edge_index,
           std::vector<int64_t>& d, std::vector<int64_t>& h,
    ...
    for (auto [u, edge_index] : adj_list_[v]) {
      ...
      if (used[u]) {
        d[v] = std::min(d[v], h[u]); //Пояснялка. Если мы уже посещали вершину, значит она - предок. Это означает, что эта вершина входит в одну комопненту со следующей. Поэтому минимум из этих двух - минимальная глубина в компоненте.
      } else {
        DFS(u, v, edge_index, d, h, used);
        d[v] = std::min(d[v], d[u]); //Если мы не посещали следующую вершину, но при ДФСЕ её мин глубина больше нашей минимальной глубины, то она - часть другой компоненты.
        if (d[u] > h[v]) {
          bridges_.push_back(edge_index);//Если минимальная глубина комп. след. вершины больше глубины точки (а значит нет грани, которая бы вернулась обратно к предкам текущей вершины), то такая ребро (u,v) является мостом.
        }
      }
    }
  }
    ...
    for (auto u : adj_list_[v]) {
      if (u != parent) {
        if (used[u]) {
          ...
          DFS(u, v, d, h, used);
          d[v] = std::min(d[v], d[u]);
          if (h[v] <= d[u] && parent != -1) { //<= т.к. точка может быть основанием компоненты (=) или началом моста (<)
            flag = true;
          }
          ++children;
        }
...
//Та же хуйня, только вместо push в бриджи мы поднимаем флаг. Если есть родители и поднят флаг - гойда. Точка сочленения. Если больше одного ребёнка и у тебя нет родителей - гойда. 